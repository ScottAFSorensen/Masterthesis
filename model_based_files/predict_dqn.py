...
from rnn.rnn import hps_sample, MDNRNN, rnn_init_state, rnn_next_state, rnn_output, rnn_output_size, sample_sequence
import online_VAE
try:
...
MIN_REPLAY_MEMORY_SIZE = 1000#0
MINIBATCH_SIZE = 32#16 #32 seem to work better
PREDICTION_BATCH_SIZE = 1
TRAINING_BATCH_SIZE = MINIBATCH_SIZE // 4
UPDATE_TARGET_EVERY = 10#10
MODEL_NAME = "tests"#"128x128_world_models_net"#"Xception"
NR_OF_ACTIONS = 6
MEMORY_FRACTION = 0.3
MIN_REWARD = -200
SPAWN_NPC = True
...
DISCOUNT = 0.99
epsilon = 0
EPSILON_DECAY =  0.9975 #0.996## 0.9975 99975
...
class CarEnv:
    ...
    def step(self, action):
        ...
        if len(self.collision_hist) != 0:
            ...
            #reward = -200
            #reward = -1#-1 - kmh*0.1
            #reward = -1*self.temp_reward
            #self.temp_reward = 0
            reward = -1
            #print("Collision, end episode: {}".format(reward))
        elif kmh > 0:#else:
            done = False
            #reward = 1
            #reward = 0.0
            #print("still going reward")
            if self.distance - self.prev_distance > 1 and self.distance >= 10:

                reward += 0.01
                #print("driving 1m, +0.01")
                #reward += 0.1
                #self.temp_reward += 0.01
                #reward += 1#(self.distance)*0.001
                #reward += (self.distance - self.prev_distance)*0.001
                #print("Step distance reward {}".format(reward))
...
class DQNAgent:
    ...
    def create_model(self):
        ...
        #model = keras.models.load_model('models/DQN_checkpoint_recent.model')
        #model = keras.models.load_model('models/128x128_test__-0.6500000000000004max_-0.9010000000000001avg_-1.1100000000000008min__1580856303.model')
        #model = keras.models.load_model('models/DQN_checkpoint_micronet_300_looksgood.model')
        #model = keras.models.load_model('models/TheStig__0.15max_-0.6950000000000001avg_-1.0min__1582843348.model')
        #model = keras.models.load_model('models/PetterSolberg__0.6800000000000004max_-0.45099999999999996avg_-0.97min__1583225598.model')

        #New attempt with 1 layer 576, z=64 using segmentation mask for loss weighting
        #Looks promising in TensorBoard
        #model = keras.models.load_model('models/1_layer_576_driver.model')

        #loading 3k model with online learning
        #model = keras.models.load_model('models/DQN_checkpoint_online_3k.model')

        #model = keras.models.load_model('models/DQN_checkpoint_slow.model')
        #model = keras.models.load_model('models/DQN_checkpoint_slow2.model')
        #model = keras.models.load_model('models/DQN_checkpoint_slow3.model')
        #Model Free Agent
        #model = keras.models.load_model('models/128x128_world_models_net_1k_ep_plswork_3__0.5700000000000003max_-0.22899999999999993avg_-0.9299999999999999min__1575472889.model')
        #Pretrained Model Free Agent trained for 1k episodes with 10 frame prediction
        #model = keras.models.load_model('models/predict_dqn__0.5000000000000002max_-0.22899999999999993avg_-0.94min__1585075076.model') #10
        #Pretrained Model Free Agent trained for 1k episodes with 20 frame prediction
        #model = keras.models.load_model('models/predict_dqn__-0.22000000000000006max_-0.843avg_-0.97min__1585088581.model') #20
        #Model Free Agent trained from scratch for 3k episodes with 10 frame prediction
        #model = keras.models.load_model('models/DQN_checkpoint_scratch_3.8k_10frames.model')
        #Model Free Agent trained from scratch for 4k episodes with 20 frame prediction
        #model = keras.models.load_model('models/predict_dqn__0.16max_-0.189avg_-0.97min__1585303600.model')
        #Model Free Agent trained from scratch 3.8k episodes with 20 frames prediction and temp = 1.15
        #model = keras.models.load_model('models/predict_dqn_3.8k_1.15_temp.model')

        #model = keras.models.load_model('predict_models/DQN_checkpoint_3968_episodes.model')
        #model = keras.models.load_model('predict_models/model_1_predict_dqn_10frames__0max_-0.8250000000000002avg_-1.280000000000001min__1588273902.model')
        #model = keras.models.load_model('predict_models/model_2_predict_dqn_10frames.model')
        model = keras.models.load_model('predict_models/model_5_predict_dqn_10frames.model')
        model.summary()
        return model
    ...
    def train(self):
        ...
        with self.graph.as_default():
            ...
            #self.model.fit(np.array(X)/255, np.array(y), batch_size=TRAINING_BATCH_SIZE, verbose=0, shuffle=False, callbacks=[self.tensorboard] if log_this_step else None) #Official
            self.model.fit(np.array(X), np.array(y), batch_size=TRAINING_BATCH_SIZE, verbose=0, shuffle=False, callbacks=[self.tensorboard] if log_this_step else None)
...
if __name__ == '__main__':
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        #try:
            ...
            act = np.full((11,), action, dtype=int)
            #print(act)
            strokes = sample_sequence(sess, rnn, hps_model, z, act, temperature=1.0, seq_len=11)
            pred_frame_current = strokes[10]
            z_current = np.expand_dims(pred_frame_current, axis=0)
            current_state = vae.decode(z_current)
            current_state = current_state[0]#*255
            #print(current_state)
            ...
            while True:
                if SHOW_PREVIEW:
                    ...
                    vis = np.concatenate((show_current/255, current_state), axis=1)
                    cv2.imshow("Agent - network input", vis)
                    cv2.waitKey(1)
                ...
                if env.distance >=10:# or dist_reach == True:
                    #dist_reach = True
                    show_current = new_state
                    episode_reward += reward
                    new_state_z = new_state.astype(np.float32)/255.0
                    new_state_z = new_state_z.reshape(1, 128, 128, 3)
                    new_z = vae.encode(new_state_z)
                    act = np.full((11,), action, dtype=int)
                    strokes = sample_sequence(sess, rnn, hps_model, new_z, act, temperature=1.0, seq_len=11)
                    pred_frame_new = strokes[10]
                    z_new = np.expand_dims(pred_frame_new, axis=0)
                    new_state = vae.decode(z_new)
                    new_state = new_state[0]#*255
                    agent.update_replay_memory((current_state, action, reward, new_state, done))
                    current_state = new_state
                    #current_state_pred = new_state_pred
                    step += 1
            ...
            if not episode % AGGREGATE_STATS_EVERY or episode == 1:
                ...
                #    agent.model.save("models/{}__{}max_{}avg_{}min__{}.model".format(MODEL_NAME, max_reward, average_reward, min_reward, int(time.time())))
                    agent.model.save("predict_models/DQN_checkpoint.model")
            # Decay epsilon
    ...
    trainer_thread.join()
    agent.model.save("predict_models/{}__{}max_{}avg_{}min__{}.model".format(MODEL_NAME, max_reward, average_reward, min_reward, int(time.time())))

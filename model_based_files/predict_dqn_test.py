...
from predict_dqn import CarEnv, MEMORY_FRACTION
import spawn_npc
#import pandas
import csv
...
#MODEL_PATH = 'predict_models/model_1_predict_dqn_10frames.model' #Done
#MODEL_PATH = 'predict_models/model_2_predict_dqn_10frames.model' #Done
#MODEL_PATH = 'predict_models/model_3_predict_dqn_10frames.model' #Done
#MODEL_PATH = 'predict_models/model_4_predict_dqn_10frames.model' #Done
MODEL_PATH = 'predict_models/model_5_predict_dqn_10frames.model'

SPAWN_NPC = True
HD_CAM = True
csv_name = "predict_data_5_30sec_100cars.csv"
with open(csv_name, 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Collision", "Type", "Distance", "Time"])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        #print(act)
        strokes = sample_sequence(sess, rnn, hps_model, z, act, temperature=0.1, seq_len=11)
        ...
        while True:
            ...
            #qs = model.predict(current_state)
            action = np.argmax(qs)
            ...
            act = np.full((11,), action, dtype=int)
            strokes = sample_sequence(sess, rnn, hps_model, new_z, act, temperature=0.1, seq_len=11)
            ...
            if done:
                ...
                if (round((time.time() - start_time)) % 60) > 1 and (round((time.time() - start_time)) % 60) <= 31 :
                    with open(csv_name, 'a+', newline='') as file:
                        writer = csv.writer(file)
                        writer.writerow([collision, collision_type, env.distance2, round((time.time() - start_time) % 60)])

...
def default_hps():
  return HyperParams(num_steps=2000, # train model for 2000 steps.
                     ...
                     input_seq_width=65,#35,    # width of our data (32 + 3 actions)
...
def sample_sequence(sess, s_model, hps, init_z, actions, temperature=1.0, seq_len=1000):
  ...
  for i in range(seq_len):
    #input_x = np.concatenate((prev_x, actions.reshape((1, 1, 1))), axis=2) #Testing
    input_x = np.concatenate((prev_x, actions[i].reshape((1, 1, 1))), axis=2) #Official
...
def rnn_next_state(rnn, z, a, prev_state):
  ...
  input_x = np.concatenate((z.reshape((1, 1, 64)), a.reshape((1, 1, 1))), axis=2)
...
def rnn_output(state, z, mode):
  ...
  if mode == MODE_ZH:
    #print(state.h[0].shape)
    return np.concatenate([z, state.h[0]], axis=None)

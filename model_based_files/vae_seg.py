...
class ConvVAE(object):
  ...
  def _build_graph(self):
    ...
    with self.g.as_default():
      ...
      #self.x = tf.placeholder(tf.float32, shape=[None, 64, 64, 3]) #For 64x64 images
      self.x = tf.placeholder(tf.float32, shape=[None, 128, 128, 3]) #For 128x128 images
      self.s = tf.placeholder(tf.float32, shape=[None, 128, 128, 3])
      # Encoder
      h = tf.layers.conv2d(self.x, 32, 4, strides=2, activation=tf.nn.relu, name="enc_conv1")
      h = tf.layers.conv2d(h, 64, 4, strides=2, activation=tf.nn.relu, name="enc_conv2")
      h = tf.layers.conv2d(h, 128, 4, strides=2, activation=tf.nn.relu, name="enc_conv3")
      h = tf.layers.conv2d(h, 256, 4, strides=2, activation=tf.nn.relu, name="enc_conv4")
      h = tf.layers.conv2d(h, 256, 4, strides=2, activation=tf.nn.relu, name="enc_conv5")
      ...
      h = tf.layers.conv2d_transpose(h, 32, 4, strides=2, activation=tf.nn.relu, name="dec_deconv4")
      self.y = tf.layers.conv2d_transpose(h, 3, 6, strides=2, activation=tf.nn.sigmoid, name="dec_deconv5")
      ...
      if self.is_training:
        ...
        # reconstruction loss
        self.r_loss = tf.reduce_sum(self.s*(tf.square(self.x - self.y)),reduction_indices = [1,2,3])
  ...
  def set_model_params(self, params):
    with self.g.as_default():
      ...
      for var in t_vars:
        ...
        assert pshape == p.shape, "inconsistent shape goddammit"

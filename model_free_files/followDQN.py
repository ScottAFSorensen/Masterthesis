...
MODEL_NAME = "7_sec_transfer_noboost_follow_model_2"
NR_OF_ACTIONS = 6
MEMORY_FRACTION = 0.3
MIN_REWARD = -200
SPAWN_NPC = False
...
class CarEnv:
    ...
    def __init__(self):
        ...
        self.pilot_car = self.blueprint_library.filter("police")[0]
        #self.pilot_car = self.blueprint_library.filter("tt")[0]
    ...
    def reset(self):
        ...
        while True:
            ...
            if (self.random_car.id == "vehicle.yamaha.yzf" or self.random_car.id == "vehicle.kawasaki.ninja"
                ...
                break
        #police_car = self.blueprint_library.filter('vehicle.dodge_charger.police')
        #Spawn car:
        ...
        while True:
            ...
            try:
                ...
                self.vehicle_ahead = self.world.spawn_actor(self.pilot_car, self.scenario_list[0])
        ...
        #Give vehicle ahead an autopilot:
        #set_autopilot(self.vehicle_ahead, enabled=True)
    ...
    def step(self, action):
        ...
        #trans = self.vehicle_ahead.get_transform()
        #print(trans.get_forward_vector())
        ...
        if len(self.collision_hist) != 0:
            ...
            reward += -1
        elif self.pilot_distance >= 1 and self.pilot_distance <= 20:
            done = False
            reward += 0.01
        ...
        if len(self.collision_hist) != 0:
            ...
            reward += -1
        elif self.pilot_distance <= 2: #used to be 4 here, probably doesn't matter
            done = False
            #reward += -0.05
            ...
            done = False #True during training
        else:
            done = False
            #reward += -0.01
        ...
        if len(self.collision_hist) != 0:
            ...
            reward += -1
        elif self.pilot_distance >= 20 and self.pilot_distance <= 30:
            done = False
            reward += 0.01
        elif self.pilot_distance >= 10 and self.pilot_distance <= 20:
            done = False
            reward += 0.02
        elif self.pilot_distance >= 1 and self.pilot_distance <= 10:
            done = False
            reward += 0.03
        else:
            done = False
        ...
        if len(self.collision_hist) != 0:
            ...
            reward += -1
        elif self.pilot_distance <= 20:
            done = False
            reward +=  (1 / self.pilot_distance)
        else:
            done = False
            reward += -0.01*self.pilot_distance#-0.01
        '''
...
class DQNAgent:
    ...
    def create_model(self):
        ...
        base_model = keras.models.load_model('model_free_models/model_free_1.model')
        ...
        base_model.layers[9].trainable = False
        #base_model.layers[10].trainable = False
        base_model.compile(loss="mse", optimizer=Adam(lr=0.0001, clipnorm = 1.0, amsgrad = True), metrics=['accuracy'])
        base_model.summary()
        return base_model
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        #try:
            ...
            while True:
                ...
                #Sticky actions: do the same action for n time steps for smoother movement
                #if step % 1 == 0:

                # This part stays mostly the same, the change is to query a model for Q values
                if np.random.random() > epsilon and env.distance >= 0:
            ...
            if len(env.collision_hist) > 0:
                ...
                if(coll_id[0:7] == "vehicle"):
                    ...
                    pass#print("You crashed in an unknown object")
